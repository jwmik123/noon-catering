...
export async function sendInvoiceEmail(invoice, pricing = null) {
  ...
  try {
    ...
    const order = {
      ...
      fullName: invoice.companyDetails?.fullName,
      orderDetails: invoice.orderDetails,
      deliveryDetails: {
        deliveryDate: invoice.orderDetails?.deliveryDate,
        deliveryTime: invoice.orderDetails?.deliveryTime,
        deliveryAddress: invoice.orderDetails?.deliveryAddress,
      },
    ...
    // Fetch sandwich options from related quote if needed
    let sandwichOptions = invoice.orderDetails?.sandwichOptions || [];
