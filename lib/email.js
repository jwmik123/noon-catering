...
const EMAIL_CONSTANTS = {
  ...
  VERIFIED_DOMAIN: "catering.noonsandwicherie.be",
  SENDER_NAME: "NOON Sandwicherie & Koffie",
  FROM_EMAIL: function () {
    return `${this.SENDER_NAME} <orders@catering.noonsandwicherie.be>`;
  },
  // ADMIN_EMAIL: ["bestel@noonsandwicherie.be", "verkoop-0795406037@abpart.clouddemat.be", "peloton@cargovelo.be"],
  EMAIL_SUBJECTS: {
    ORDER_CONFIRMATION: (invoiceNumber) => `Orderbevestiging - ${invoiceNumber}`,
    INVOICE: (invoiceNumber) => `Factuur NOON Sandwicherie & Koffie - ${invoiceNumber}`,
  },
};
...
function getOrderConfirmationHtml({
  ...
  // Helper function to find sandwich name by ID, prefixed with category
  const getSandwichName = (sandwichId) => {
    const sandwich = sandwichOptions.find((s) => s._id === sandwichId);
    if (!sandwich) return "Onbekend item";
      ...
      <body>
        <div class="container">
          ...
          <div class="header">
            ...
            <div class="order-id">Factuur nr: ${invoiceNumber}</div>
              ...
              <div class="section">
                ...
                <div class="delivery-details">
                  ...
                  <div class="delivery-row">
                    ...
                    ${(orderDetails?.additionalPhoneNumbers || []).filter(Boolean).map(p => `<br><span style="margin-left:8px">${p}</span>`).join('')}
                  </div>
                  <div class="delivery-row">
                  ...
                  `}
                  ${(orderDetails?.additionalEmails || []).filter(Boolean).length > 0 ? `
                  <div class="delivery-row">
                    <span class="delivery-label">Extra e-mailadressen:</span><br>
                    ${(orderDetails.additionalEmails).filter(Boolean).join('<br>')}
                  </div>
                  ` : ''}
                  <div class="delivery-row">
                    ...
                    ? Object.entries(orderDetails.customSelection || {})
                      ...
                      (orderDetails.addDrinks && orderDetails.drinks
                        ? `
                          ...
                          <tr>
                            <td>Verse Jus</td>
                            <td>${orderDetails.drinks.freshOrangeJuice}x</td>
                            <td>-</td>
                            <td>-</td>
                            <td>-</td>
                            <td>€${(orderDetails.drinks.freshOrangeJuice * 3.35).toFixed(2)}</td>
                          </tr>
                          ` : ""}
                      ...
                      (orderDetails.addSoup && orderDetails.soup
                        ...
                        `
                        : "") +
                      (orderDetails.addDesserts && orderDetails.desserts
                        ? `
                          ${orderDetails.desserts.desserts > 0 ? `
                          <tr>
                            <td>Desserts</td>
                            <td>${orderDetails.desserts.desserts}x</td>
                            <td>-</td>
                            <td>-</td>
                            <td>-</td>
                            <td>€${(orderDetails.desserts.desserts * (pricing?.desserts?.desserts || 3.50)).toFixed(2)}</td>
                          </tr>
                          ` : ""}
                          ${orderDetails.desserts.cookies > 0 ? `
                          <tr>
                            <td>Koekjes</td>
                            <td>${orderDetails.desserts.cookies}x</td>
                            <td>-</td>
                            <td>-</td>
                            <td>-</td>
                            <td>€${(orderDetails.desserts.cookies * (pricing?.desserts?.cookies || 2.50)).toFixed(2)}</td>
                          </tr>
                          ` : ""}
                        `
                        : "")
          ...
          <div class="price-section">
            <div class="price-row">Subtotaal${(amountData.discount || 0) > 0 ? ' (excl. korting)' : ''}: €${((amountData.subtotal || 0) + (amountData.discount || 0)).toFixed(2)}</div>
            ${(amountData.discount || 0) > 0 ? `<div class="price-row" style="color: #16a34a;">Korting${couponCode ? ` (${couponCode})` : ''}: -€${(amountData.discount || 0).toFixed(2)}</div><div class="price-row">Subtotaal na korting: €${(amountData.subtotal || 0).toFixed(2)}</div>` : ''}
...
export async function sendOrderConfirmation(order, isInvoiceEmail = false, pricing = null) {
  ...
  try {
    ...
    const adminEmails = ["communicatie@noonsandwicherie.be"];
    if (!isPickup && process.env.NODE_ENV === "production") {
      adminEmails.push("peloton@cargovelo.be");
    }
    // For Mollie payments, CC abpart since the invoice is sent immediately.
    // For invoice payment method, abpart receives the invoice via the cron job instead.
    if (order.orderDetails?.paymentMethod !== "invoice") {
      adminEmails.push("verkoop-0795406037@abpart.clouddemat.be");
    }
    ...
    console.log(
      "- Subject:",
      isInvoiceEmail
        ? EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber)
        : EMAIL_CONSTANTS.EMAIL_SUBJECTS.ORDER_CONFIRMATION(order.invoiceNumber)
    );

    const allRecipients = [order.email, ...(order.additionalEmails || [])].filter(Boolean);
 ...
 */
export async function sendInvoiceEmail(invoice, pricing = null, options = {}) {
  ...
  try {
    ...
    // Prefer invoice.companyDetails.address (editable in Sanity Studio) over the
    // flat orderDetails snapshot fields, then fall back to the snapshot.
    ...
    const invoiceDetails = {
      ...
      address: hasSanityBillingAddress
        ? {
            ...
            houseNumberAddition: companyBillingAddress.houseNumberAddition || "",
    ...
    // Enrich companyDetails: prefer Sanity-editable fields, fall back to orderDetails snapshot
    ...
    // Invoice emails: cron job sends to verkoop (accounting), Studio sends to customer only
    const ccEmails = options.customerOnly ? [] : ["verkoop-0795406037@abpart.clouddemat.be"];

    const allInvoiceRecipients = [order.email, ...(orderDetails.additionalEmails || [])].filter(Boolean);

    console.log("Sending invoice email:");
    console.log("- From:", fromEmail);
    console.log("- To:", allInvoiceRecipients);
    console.log("- CC:", ccEmails);
    console.log("- Subject:", EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber));

    const emailData = {
      from: fromEmail,
      to: allInvoiceRecipients,
      cc: ccEmails,
      reply_to: "bestel@noonsandwicherie.be",
      headers: {
        "Reply-To": "bestel@noonsandwicherie.be",
      },
      subject: EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber),
      html: emailHtml,
      attachments: [
        {
          filename: `factuur-${order.invoiceNumber}.pdf`,
          content: pdfBuffer,
        },
      ],
    };
...
/**
 * Send cancellation emails when an order is cancelled from Sanity Studio.
 * - Customer receives a cancellation notification.
 * - Verkoop receives a credit note summary for accounting.
 */
export async function sendCancellationEmail(invoice) {
  ...
  const customerName = companyDetails.name || orderDetails.name || "klant";
  const fromEmail = EMAIL_CONSTANTS.FROM_EMAIL();

  if (!customerEmail) {
    console.error("No customer email found for cancellation:", invoice._id);
    return { success: false, error: "No email address found for this invoice" };
  }
  ...
  const sharedStyle = `
    <style>
      ...
      .header h1 { color: #524a98; }
      .order-id { color: #666; font-size: 14px; margin: 6px 0; }
      .section-title { color: #524a98; font-size: 16px; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 2px solid #524a98; }
      .box { background-color: #f9f9f9; padding: 15px; border-radius: 5px; border: 1px solid #524a98; margin-bottom: 20px; }
      .row { margin-bottom: 6px; }
      .label { font-weight: bold; color: #524a98; }
      .price-section { margin-top: 20px; text-align: right; }
      .price-row { margin: 4px 0; }
      .total { font-size: 16px; font-weight: bold; color: #524a98; margin-top: 8px; }
      .footer { margin-top: 40px; padding-top: 20px; border-top: 1px solid #524a98; font-size: 12px; color: #666; text-align: center; }
    </style>
  `;

  // --- Customer cancellation email ---
  const customerHtml = `
    <html><head><meta charset="utf-8">${sharedStyle}</head><body>
      <div class="container">
        <div class="header">
          <h1>Bestelling geannuleerd</h1>
          <div class="order-id">Bestelling: ${invoiceNumber}</div>
          <div class="order-id">Datum: ${new Date().toLocaleDateString("nl-NL")}</div>
        </div>
        <div style="background-color:#f0f7ff;padding:15px;border-radius:5px;margin-bottom:20px;border:1px solid #524a98;">
          <p style="margin:0;color:#524a98;font-weight:500;text-align:center;">Beste ${customerName},</p>
          <p style="margin:10px 0 0;color:#524a98;font-weight:500;text-align:center;">
            Uw bestelling is geannuleerd. Heeft u vragen? Neem gerust contact met ons op.
          </p>
        </div>
        <div class="box">
          <div class="section-title">Bestelgegevens</div>
          <div class="row"><span class="label">Bestelling:</span> ${invoiceNumber}</div>
          <div class="row"><span class="label">Leverdatum:</span> ${orderDetails.deliveryDate ? new Date(orderDetails.deliveryDate).toLocaleDateString("nl-NL") : "—"}</div>
          <div class="row"><span class="label">Totaalbedrag:</span> €${(amount.total || 0).toFixed(2)}</div>
        </div>
        <div class="footer">
          <p>NOON Sandwicherie & Koffie</p>
          <p>Keizer Leopoldstraat 1, 9000 Gent, België</p>
    ...
    <html><head><meta charset="utf-8">${sharedStyle}</head><body>
      <div class="container">
        ...
        <div class="box">
          ...
          <div class="row"><span class="label">Naam:</span> ${customerName}</div>
          <div class="row"><span class="label">E-mail:</span> ${customerEmail}</div>
          ${companyDetails.btwNumber ? `<div class="row"><span class="label">BTW-nummer:</span> ${companyDetails.btwNumber}</div>` : ""}
          ${invoice.referenceNumber ? `<div class="row"><span class="label">Referentie:</span> ${invoice.referenceNumber}</div>` : ""}
        </div>
        <div class="box">
          <div class="section-title">Bestelgegevens</div>
          <div class="row"><span class="label">Bestelling:</span> ${invoiceNumber}</div>
          <div class="row"><span class="label">Leverdatum:</span> ${orderDetails.deliveryDate ? new Date(orderDetails.deliveryDate).toLocaleDateString("nl-NL") : "—"}</div>
          <div class="row"><span class="label">Betaalmethode:</span> ${orderDetails.paymentMethod === "online" ? "Online (Mollie)" : "Factuur"}</div>
        </div>
        <div class="price-section">
          <div class="price-row">Subtotaal: €${(amount.subtotal || 0).toFixed(2)}</div>
          ${(amount.delivery || 0) > 0 ? `<div class="price-row">Levering: €${(amount.delivery || 0).toFixed(2)}</div>` : ""}
          <div class="price-row">BTW Voeding (6%): €${(amount.foodVAT || 0).toFixed(2)}</div>
          ${(amount.deliveryVAT || 0) > 0 ? `<div class="price-row">BTW Levering (21%): €${(amount.deliveryVAT || 0).toFixed(2)}</div>` : ""}
          <div class="price-row total">Te crediteren totaal: €${(amount.total || 0).toFixed(2)}</div>
        </div>
        <div class="footer">
          <p>NOON Sandwicherie & Koffie — intern gebruik</p>
        </div>
      </div>
    </body></html>
  `;
  ...
  try {
    const [customerResult, verkoopResult] = await Promise.all([
      resend.emails.send({
        from: fromEmail,
        to: customerEmail,
        reply_to: "bestel@noonsandwicherie.be",
        subject: `Bestelling geannuleerd - ${invoiceNumber}`,
        html: customerHtml,
      }),
      resend.emails.send({
        from: fromEmail,
        to: "verkoop-0795406037@abpart.clouddemat.be",
        subject: `CREDITNOTA - ${invoiceNumber}`,
        html: verkoopHtml,
      }),
    ]);

    if (customerResult.error) {
      console.error("Failed to send customer cancellation email:", customerResult.error);
      return { success: false, error: customerResult.error.message };
    }
    if (verkoopResult.error) {
      console.error("Failed to send verkoop credit note email:", verkoopResult.error);
      return { success: false, error: verkoopResult.error.message };
    }

    console.log("Cancellation emails sent successfully");
    console.log("========= CANCELLATION EMAILS COMPLETED =========");
    return { success: true };
  } catch (error) {
