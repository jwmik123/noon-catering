  ...
  try {
    ...
    const emailData = {
      ...
      subject: isInvoiceEmail
        ? EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber || order.quoteId)
        : EMAIL_CONSTANTS.EMAIL_SUBJECTS.ORDER_CONFIRMATION(order.quoteId),
      html: emailHtml,
    };
...
export async function sendInvoiceEmail(invoice, pricing = null, options = {}) {
  ...
  try {
    ...
    console.log("- Subject:", EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber || order.quoteId));
    ...
    const emailData = {
      ...
      subject: EMAIL_CONSTANTS.EMAIL_SUBJECTS.INVOICE(order.invoiceNumber || order.quoteId),
      html: emailHtml,
      attachments: [
        {
          filename: `factuur-${order.invoiceNumber || order.quoteId}.pdf`,
          content: pdfBuffer,
        },
      ],
    };
...
export async function sendCancellationEmail(invoice) {
  ...
  const quoteId = invoice.quoteId || invoice._id;
    ...
    <html><head><meta charset="utf-8">${sharedStyle}</head><body>
      <div class="container">
        <div class="header">
          ...
          <div class="order-id">Bestelling: ${quoteId}</div>
          <div class="order-id">Datum: ${new Date().toLocaleDateString("nl-NL")}</div>
        </div>
        ...
        <div class="box">
          ...
          <div class="row"><span class="label">Bestelling:</span> ${quoteId}</div>
  ...
  try {
    const [customerResult, verkoopResult] = await Promise.all([
      resend.emails.send({
        ...
        to: customerEmail,
        reply_to: "bestel@noonsandwicherie.be",
        subject: `Bestelling geannuleerd - ${quoteId}`,
        html: customerHtml,
      }),
      resend.emails.send({
        from: fromEmail,
        to: "verkoop-0795406037@abpart.clouddemat.be",
        subject: `CREDITNOTA - ${quoteId}`,
        html: verkoopHtml,
      }),
    ]);
