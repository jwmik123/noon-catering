 ...
 */
export const calculateVATBreakdown = (foodSubtotal, deliverySubtotal, discount = 0) => {
  // Round inputs to 2 decimals
  const roundedFood = round2(Math.max(0, foodSubtotal - discount));
 ...
 */
export const calculateTotalWithVAT = (foodSubtotal, deliveryCost = 0, discount = 0) => {
  const breakdown = calculateVATBreakdown(foodSubtotal, deliveryCost, discount);
 ...
 */
export const formatVATBreakdown = (foodSubtotal, deliveryCost = 0, discount = 0) => {
  const breakdown = calculateVATBreakdown(foodSubtotal, deliveryCost, discount);
  ...
  return {
    ...
    discount: discount > 0 ? `€${breakdown.discount.toFixed(2)}` : null,
    breakdown: breakdown
  };
};