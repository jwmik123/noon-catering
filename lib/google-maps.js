...
export const DELIVERY_PRICING = {
  RANGES: [
    ...
    { maxKm: 7, cost: 30, label: "5-7km" },   // 5-10km: €17
    { maxKm: Infinity, cost: 31, label: "7km+" } // 5km+: €25
  ],
  FREE_DELIVERY_THRESHOLD: 350 // Free delivery for orders above €100
};
...
// Calculate actual driving distance using Google Maps Directions API (most accurate for delivery routing)
      ...
      .then((maps) => {
        try {
          ...
          directionsService.route({
            origin: new maps.LatLng(point1.lat, point1.lng),
            destination: new maps.LatLng(point2.lat, point2.lng),
            ...
            if (status === 'OK') {
              // Use the first route (Google's recommended route)
